...
# Définir les variables nécessaires (à ajuster selon votre cas)
profile_cone_up = ...
profile_cone_dn = ...
profile_all_lf = ...
profile_all_r = ...
...
profile_bridge_dn = ...
image_name_prefix = ...
figure_amplification_factor = ...
figure_width_in_inches = ...
figure_height_in_inches = ...
...
# Sélectionner les parties restreintes au cône
y_cone_updn_indexes = np.where(y_array >= profile_cone_up)
y_array_restricted_to_cone = y_array[y_cone_updn_indexes].reshape((-1, 1))
x_left_restricted_to_cone = x_from_y_left[y_cone_updn_indexes]

# Régression linéaire sur la partie gauche
model_left = LinearRegression()
model_left.fit(y_array_restricted_to_cone, x_left_restricted_to_cone)

# Calculer l'angle beta_solid_left à partir de la pente
beta_solid_left = np.arctan(model_left.coef_)[0]
alpha_left = -beta_solid_left

# Calculer la symétrie de l'angle pour la partie droite
...
# Calcul de la symétrie des coordonnées pour la partie droite
c = (profile_all_lf + profile_all_r) / 2
x_right_symmetry = 2 * c - model_left.predict(y_array_restricted_to_cone)

# Plot des résultats
fig, ax = plt.subplots(figsize=(figure_amplification_factor * figure_width_in_inches, figure_amplification_factor * figure_height_in_inches), dpi=150)

# Affichage de l'image en arrière-plan
ax.imshow(A_graph, cmap='gray', extent=(profile_all_lf, profile_all_r, profile_bridge_up, profile_cone_dn), aspect='equal')

# Tracé des frontières
ax.plot(x_left_restricted_to_cone, y_array_restricted_to_cone, 'r-', markersize=5, label='Left boundary')
ax.plot(x_right_symmetry, y_array_restricted_to_cone, 'c-', markersize=5, label='Right boundary (Symmetry)')

# Configuration des axes
ax.set_xlim(profile_all_lf, profile_all_r)
ax.set_ylim(profile_bridge_up, profile_cone_dn)
ax.axis('equal')
ax.axis('tight')
ax.invert_yaxis()
...
ax.set_title('Symmetrized boundaries', size=13)

# Sauvegarde et affichage de la figure
plt.savefig(os.path.join(png_directory, image_name_prefix + '_left_right_symmetry_boundaries.png'))
plt.show()
plt.close()
